...
### `mcp__nyaascripts__devcontainerChat`
...
- `agent` is usually `claude`. If not installed, use `devcontainerExec` to run `which claude cursor copilot codex` to discover available agents.

### `mcp__nyaascripts__devcontainerExec`
...
- Choose whether to use `devcontainerChat` (full agent conversation) or `devcontainerExec` (quick shell command) based on the task.
...
When `devcontainerChat` returns `status: "running"`:
1. Poll with `jobId`
