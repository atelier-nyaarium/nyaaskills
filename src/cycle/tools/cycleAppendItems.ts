...
`.trim(),
	...
	dedupe: z
		...
		.describe(
			`
Keep only incoming items not already in the queue (normalized: trimmed, trailing slashes stripped), comparing against the active (non-deferred) items, so a deliberately re-queued deferred item is kept. Reports the duplicate count.
`.trim(),
		),
});
...
export const cycleAppendItems = {
	...
	description: `
Append items to a running items-mode queue. Use to load a large job in chunks, or to add work discovered mid-run. With \`dedupe\`, only items not already queued are added. Safe to interleave with stepping.
`.trim(),
	operation: "appending items to a queue",
	schema,
	async handler(cwd: string, args: z.infer<typeof schema>) {
		const { name, items: newItems, dedupe } = schema.parse(args);
		...
		for (let attempt = 0; attempt < 3; attempt++) {
			...
			const appended = appendItems(progress.items, progress.deferredItemIndexes, newItems, dedupe);
			const next: StoredProgress = { ...progress, items: appended.items };
			try {
				writeProgress(planFile, next);
				const duplicatesNote = appended.duplicates
					? ` (${appended.duplicates} duplicate(s) already queued)`
					: "";
				const result = {
					name,
					plan,
					...
					instructions: `Appended ${appended.added} item(s)${duplicatesNote}; queue is now ${appended.items.length}. Keep going - loop to the next batch; do not stop unless finishing or blocked.`,
				};
