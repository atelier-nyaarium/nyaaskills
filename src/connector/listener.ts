...
import { type ClientData, addClient, getAllClients, getClient, removeClient } from "./sessions.ts";
...
export function invokeOnClient(shortHash: string, tool: string, args: Record<string, unknown>): Promise<unknown> {
	...
	if (!client) {
		return Promise.reject(new Error(`Client ${shortHash} not connected. Call mcpConnectorStatus for active clients.`));
	}
