...
import { z } from "zod";
import { invokeOnClient } from "./connector/listener.ts";
import { registerStubTool, textResult } from "./utils.ts";
...
export async function registerProjectTools(mcpServer: McpServer, projectName: string, connectorDir: string): Promise<void> {
	...
	if (typeof schemaFn !== "function") {
		...
		registerStubTool(mcpServer, "projectMcpConnectorStatus", `Project MCP connector failed to load. Call this tool for error details.`, () => msg);
		return;
	}
			...
			async (args) => {
				try {
					...
					if (!clientId || typeof clientId !== "string") {
						return textResult(`clientId is required. Call mcpConnectorStatus to get connected client IDs.`, true);
					}

					const result = await invokeOnClient(clientId, tool.name, toolArgs);
					return textResult(JSON.stringify(result, null, 2));
				} catch (error) {
					return textResult(JSON.stringify({ errors: [{ message: (error as Error).message }] }, null, 2), true);
				}
			},
		);
	}
