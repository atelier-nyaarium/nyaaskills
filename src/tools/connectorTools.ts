...
import { z } from "zod";
import { getAuthToken, getListenerState, restartWithTls, restartWithoutTls, setAuthToken } from "../connector/listener.ts";
import { getAllClients, getClient } from "../connector/sessions.ts";
import { generateCaCert, generateServerCert } from "../connector/tls.ts";
import { textResult } from "../utils.ts";

const SCHEMA_TEMPLATE = `/**
 * MCP Schema - Project-MCP-Loader
 *
...
\t\t\ttitle: "Foo",
\t\t\tdescription:
\t\t\t\t"A simple test tool for validating the project-mcp-connector works. Accepts a numeric 'bar' parameter and echoes it back.",
\t\t\tschema: z.object({
...
export function registerConnectorTools(mcpServer: McpServer, projectName: string, connectorDir: string, port: number): void {
	...
	mcpServer.registerTool(
		...
		async () => {
			try {
				if (!getAuthToken()) {
					return textResult(`Cannot open to public without authentication. Run mcpConnectorGenerateToken first.`, true);
				}
	...
	if (!hasSchema) {
		mcpServer.registerTool(
			...
			async () => {
				...
				return textResult(`Created ${schemaPath} with example schema.\nUse /mcp to restart this MCP server to load it.`);
			},
		);
	}
}
